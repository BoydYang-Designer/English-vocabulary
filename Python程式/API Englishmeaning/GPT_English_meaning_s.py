...
def generate_english_details(client, word):
    global total_prompt_tokens, total_completion_tokens
    prompt = f"""
Give a short and learner-friendly explanation for the English word '{word}'.

Rules:
- Only include real and common parts of speech (noun, verb, adjective).
- Start each section with a header like:
  {word.capitalize()} is a noun : singular / plural : with the following meanings:
- Definitions must be short (1–2 lines).
- Give exactly 1 example sentence for each meaning, starting with "E.g."
- At the end, add:
  Summary:
  [short summary]
...
Choose the most suitable primary and secondary categories.

{CATEGORY_GUIDE}

Output in JSON with keys: "category1", "category2".
"""
    if NEW_OPENAI_SDK:
        response = client.chat.completions.create(
            model="gpt-4o",
