...
def main():
    ...
    count = simpledialog.askinteger("輸入", "要處理幾個單字？", minvalue=1)
    if not count:
        print("未輸入處理數量，程式結束。")
        return
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        # 如果有單字，且發音欄位是空的，就加入待處理清單
        if word and (pron_cell.value is None or str(pron_cell.value).strip() == ""):
            words_to_process.append(row)
    
    # 只取使用者指定的數量
    words_to_process = words_to_process[:count]
    print(f"\n找到 {len(words_to_process)} 個需要處理的單字...")
